...
public class Solution1 {
    ...
    /*
     * 
     * This solution does'nt give TLE due to the nature of parameters used here
     * i.e target is decreasing always.
     */
    int solve(int[][] q, int i, int target, int k, int[][] dp) {
        ...
        if (q[k][0] <= i && i <= q[k][1])
            res = Math.min(res, solve(q, i, target - q[k][2], k + 1, dp)); // pick the val if its range is in the range
                                                                           // of target index
